...
def warmup_analytics_caches():
    ...
    try:
        ...
        try:
            ...
            logger.info("[1/9] Summary cache ready (%.2fs)", time.perf_counter() - start)
            start = time.perf_counter()
            analytics.get_top_pickup_zones(top_n=10, borough=None, date=None, distance=None, fare=None, db=db)
            logger.info("[2/9] Top Pickup Zones (10) cache ready (%.2fs)", time.perf_counter() - start)
            start = time.perf_counter()
            analytics.get_top_pickup_zones(top_n=15, borough=None, date=None, distance=None, fare=None, db=db)
            logger.info("[3/9] Top Pickup Zones (15) cache ready (%.2fs)", time.perf_counter() - start)
            start = time.perf_counter()
            analytics.get_top_dropoff_zones(top_n=10, borough=None, date=None, distance=None, fare=None, db=db)
            logger.info("[4/9] Top Dropoff Zones cache ready (%.2fs)", time.perf_counter() - start)
            start = time.perf_counter()
            analytics.get_fare_distribution(borough=None, date=None, distance=None, fare=None, db=db)
            logger.info("[5/9] Fare Distribution cache ready (%.2fs)", time.perf_counter() - start)
            start = time.perf_counter()
            analytics.get_revenue_by_borough(borough=None, date=None, distance=None, fare=None, db=db)
            logger.info("[6/9] Revenue By Borough cache ready (%.2fs)", time.perf_counter() - start)
            start = time.perf_counter()
            analytics.get_revenue_trends(borough=None, date=None, distance=None, fare=None, db=db)
            logger.info("[7/9] Revenue Trends cache ready (%.2fs)", time.perf_counter() - start)
            start = time.perf_counter()
            analytics.get_average_fare(borough=None, date=None, distance=None, fare=None, db=db)
            logger.info("[8/9] Average Fare cache ready (%.2fs)", time.perf_counter() - start)
            start = time.perf_counter()
            analytics.get_average_distance(borough=None, date=None, distance=None, fare=None, db=db)
            logger.info("[9/9] Average Distance cache ready (%.2fs)", time.perf_counter() - start)
            logger.info("Analytics cache warmup completed in %.2fs", time.perf_counter() - total_start)
            logger.info("=" * 70)
        finally:
            db.close()
...
# Frontend routes — serves the frontend directly from FastAPI
# with short, clean URLs instead of long file paths.
